          ...
          <div className="md:hidden">
            <div className="flex flex-col items-center gap-8">
              ...
              <div className="relative w-full flex items-center justify-center">
                ...
                <button
                  onClick={() => setPhoneIdx(i => Math.max(0, i - 1))}
                  disabled={phoneIdx === 0}
                  className={`absolute left-0 z-10 w-10 h-10 rounded-xl ${phoneIdx === 0
                ...
                <PhoneMockup screen={PROJECT_SCREENS[phoneIdx]} />

                <button
                  onClick={() =>
                    setPhoneIdx(i =>
                      Math.min(PROJECT_SCREENS.length - 1, i + 1)
                    )
                  }
                  disabled={phoneIdx === PROJECT_SCREENS.length - 1}
                  className={`absolute right-0 z-10 w-10 h-10 rounded-xl ${phoneIdx === PROJECT_SCREENS.length - 1
                    ? "bg-gray-800 text-gray-600"
                    : "bg-red-600 text-white"
                    }`}
                >
              ...
              <div className="text-center">
                <p className="text-white font-black text-xl">
                  {PROJECT_SCREENS[phoneIdx].label}
                </p>

                <p className="text-gray-500 text-sm mt-1">
                  {PROJECT_SCREENS[phoneIdx].desc}
                </p>
                ...
                <div className="flex gap-2 justify-center mt-4">
                  {PROJECT_SCREENS.map((_, i) => (
                    <button
                      ...
                      style={{
                        width: phoneIdx === i ? 28 : 8,
                        height: 8,
                        background: phoneIdx === i ? "#dc2626" : "#374151",
                      }}
                    />
                  ))}
                </div>
          ...
          <div className="hidden md:block">
            <div className="flex flex-col items-center gap-8">
              <div className="relative w-full flex items-center justify-center gap-4">
                ...
                <button
                  onClick={() => setPhoneIdx(i => Math.max(0, i - 1))}
                  className={`absolute left-0 z-10 w-11 h-11 rounded-xl flex items-center justify-center font-bold transition-all ${phoneIdx === 0
                    ? "bg-gray-800 text-gray-600"
                ...
                <button
                  ...
                  className={`absolute right-0 z-10 w-11 h-11 rounded-xl flex items-center justify-center font-bold transition-all ${phoneIdx === PROJECT_SCREENS.length - 1
                    ? "bg-gray-800 text-gray-600"
                    : "bg-red-600 text-white hover:bg-red-700"
                    }`}
                  disabled={phoneIdx === PROJECT_SCREENS.length - 1}
